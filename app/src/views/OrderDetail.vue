    ...
    <div v-else-if="order && deal" class="order-content">
              ...
              <div class="product-price">
                ...
                <span v-if="product.pricing_type === 'bundled_weight'" class="price-value" :class="{ 'is-sale': isOnSale(product) }">
                  <template v-if="getOrderItemForProduct(product) && getOrderItemForProduct(product).final_weight">
                    ${{ moneyPlain(parseFloat(paidUnitRate(product) || 0) * parseFloat(getOrderItemForProduct(product).final_weight)) }}
                  </template>
                  <template v-else>
      ...
      <div class="payment-section">
        ...
        <div v-else class="card-bind-panel">
          ...
          <label v-if="canEditPaymentDelivery" class="card-email-label">
            邮箱（用于收据，选填）
            <input
...
export default {
  ...
  methods: {
    ...
    async loadOrder() {
      ...
      try {
        ...
        if (this.order.items && this.order.items.length > 0) {
          this.applyOrderItemsToSelection(this.order.items)
        }
    ...
    async updateOrder() {
      ...
      try {
        ...
        if (updatedOrder.items && updatedOrder.items.length > 0) {
          ...
          this.applyOrderItemsToSelection(updatedOrder.items)
        }
    ...
    async syncCardSetupReturn() {
      ...
      if (setup || this.$route.query.session_id || this.$route.query.paid != null) {
    ...
    async startCardSetup() {
      ...
      try {
        const { data } = await apiClient.post('/payments/setup-session', {
          ...
          return_path: this.$route.path || `/orders/${this.order?.id}`
        })
        if (data?.url) {
