<template>
  <nav class="bottom-nav" :class="{ 'bottom-nav--six': navItems.length > 5 }">
    <router-link
...
export default {
  ...
  computed: {
    navItems() {
      ...
      if (this.authStore.isInfluencer) {
        items.push({
          path: '/influencer',
          label: '推广收益',
  ...
  methods: {
    isActive(item) {
      // 团购下单: home is deal detail (or placeholder)
      if (item.path === '/') {
      ...
      if (item.path === '/influencer') {
        return this.$route.path === '/influencer' || this.$route.path.startsWith('/influencer/')
      }
