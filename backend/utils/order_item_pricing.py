...
def get_display_price_from_config(pricing_type, pricing_data):
    ...
    if pricing_type == 'per_item':
        if pricing_data.get('price') is None:
            return None
        return _safe_float(pricing_data.get('price'), 0)
    if pricing_type == 'weight_range':
        ranges = pricing_data.get('ranges') or []
        if ranges:
            return _safe_float(ranges[0].get('price'), 0)
        return None
    if pricing_type == 'unit_weight':
        if pricing_data.get('price_per_unit') is None:
            return None
        return _safe_float(pricing_data.get('price_per_unit'), 0)
    if pricing_type == 'bundled_weight':
        ...
        return price_per_unit * ((min_weight + max_weight) / 2)
    return None
...
def compute_price_from_config(pricing_type, pricing_data, quantity=1, final_weight=None):
        ...
        if fw is not None and ranges:
            ...
            for range_item in ranges:
                ...
                if fw >= min_weight and (max_weight is None or fw < max_weight):
                    ...
                    break
            unit_price = matched_price if matched_price is not None else _safe_float(ranges[0].get('price'), 0) if ranges else 0
        else:
            unit_price = _safe_float(ranges[0].get('price'), 0) if ranges else 0
        total_price = unit_price * quantity
    ...
    return _safe_float(unit_price, 0), _safe_float(total_price, 0)
...
def build_priced_order_item(
    ...
    """
    variant, err = resolve_variant(product, variant_id)
    if err:
...
def recalculate_order_totals(order):
    """Recompute order subtotal, shipping, total, and points from line items."""
    from decimal import Decimal
    from models.product import Product
    ...
    for oi in order.items:
        subtotal += Decimal(str(oi.total_price or 0))
    ...
    order.points_earned = int((subtotal + tax) * 100)
...
def priced_items_from_request(items, unavailable_by_item_id=None):
    ...
    for item_data in items:
        ...
        subtotal += Decimal(str(priced['total_price']))
